...
public class TimerActionConfigurationComposite implements SelectionListener {
	...
	private void create() {
		actionTypeLabel = widgetFactory.createLabel(parent, "动作类型");
		actionTypeCombo = widgetFactory.createCCombo(parent);
		actionTypeCombo.setItems( new String[] {"<空>", "动作", "脚本"});
		actionTypeCombo.addSelectionListener(this);
	...
	private void handleActionTypeComboSelected() {
		if ("动作".equals(actionTypeCombo.getText())) {
		...
		} else if ("脚本".equals(actionTypeCombo.getText())) {
		...
		actionComposite.setVisible("动作".equals(actionTypeCombo.getText()));
		scriptComposite.setVisible("脚本".equals(actionTypeCombo.getText()));
	}
